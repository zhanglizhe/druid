...
public class CoordinatorServerViewTest extends CuratorTestBase
{
  ...
  public void testSingleServerAddedRemovedSegment() throws Exception
  {
    ...
    Assert.assertFalse(segmentLoadInfo.getServers().isEmpty());
    Assert.assertEquals(druidServer.getMetadata(), Iterables.getOnlyElement(segmentLoadInfo.getServers()));
  ...
  )
  {
    ...
    for (int i = 0; i < expected.size(); ++i) {
      ...
      SegmentLoadInfo segmentLoadInfo =  actualPartitionHolder.iterator().next().getObject();
      Assert.assertFalse(segmentLoadInfo.getServers().isEmpty());
      Assert.assertEquals(expectedPair.rhs.rhs.lhs.getMetadata(),Iterables.getOnlyElement(segmentLoadInfo.getServers()));
    }
  }
