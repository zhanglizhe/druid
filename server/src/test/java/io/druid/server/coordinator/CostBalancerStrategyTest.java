...
public class CostBalancerStrategyTest
{
  ...
  public void testComputeJointSegmentCost()
  {
    ...
    Assert.assertEquals(
        CostBalancerStrategy.COST_FACTOR * CostBalancerStrategy.intervalCost(
  ...
  public void testIntervalCost() throws Exception
  {
    ...
    Assert.assertEquals(CostBalancerStrategy.intervalCost(1, 1, 3),
                        ...
                        CostBalancerStrategy.intervalCost(1, 2, 3), 1e-4);

    Assert.assertEquals(CostBalancerStrategy.intervalCost(2, 1, 3),
                        CostBalancerStrategy.intervalCost(2, 1, 2) +
                        CostBalancerStrategy.intervalCost(2, 2, 3), 1e-4);

    Assert.assertEquals(CostBalancerStrategy.intervalCost(3, 1, 2),
                        CostBalancerStrategy.intervalCost(1, 1, 2) +
                        CostBalancerStrategy.intervalCost(1, 0, 1) +
                        CostBalancerStrategy.intervalCost(1, 1, 2), 1e-4);

    // no overlap [0, 1) [1, 2)
    Assert.assertEquals(0.3995764, CostBalancerStrategy.intervalCost(1, 1, 2), 1e-4);
    // no overlap [0, 1) [-1, 0)
    Assert.assertEquals(0.3995764, CostBalancerStrategy.intervalCost(1, -1, 0), 1e-4);

    // exact overlap [0, 1), [0, 1)
    Assert.assertEquals(0.7357589, CostBalancerStrategy.intervalCost(1, 0, 1), 1e-4);
    // exact overlap [0, 2), [0, 2)
    Assert.assertEquals(2.270671, CostBalancerStrategy.intervalCost(2, 0, 2), 1e-4);
    // partial overlap [0, 2), [1, 3)
    Assert.assertEquals(1.681908, CostBalancerStrategy.intervalCost(2, 1, 3), 1e-4);
    // partial overlap [0, 2), [1, 2)
    Assert.assertEquals(1.135335, CostBalancerStrategy.intervalCost(2, 1, 2), 1e-4);
    // partial overlap [0, 2), [0, 1)
    Assert.assertEquals(1.135335, CostBalancerStrategy.intervalCost(2, 0, 1), 1e-4);
    // partial overlap [0, 3), [1, 2)
    Assert.assertEquals(1.534912, CostBalancerStrategy.intervalCost(3, 1, 2), 1e-4);
  }

  private DataSegment createSegment(DateTime t, String dataSource)
  {
